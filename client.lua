...
function ToggleCodeRunning()
    StartLSD = not StartLSD
end

local startss = true
local sabrkn = true

RegisterNetEvent("Sky_LSDRecoil:RunLSD", function ()

    local cooldownDuration = Config.Time * 1000
	if Config.Use1 then
	if sabrkn then
    Wait(1000)
    TriggerServerEvent('Sky_LSDRecoil:removelsdinventory', 'lsd', 1)
    
    if startss then

        ToggleCodeRunning()
        local message = StartLSD and "Sky_LSDRecoil : LSD faale" or Config.Timetext
        TriggerEvent("chatMessage", Config.SystemMessage, {255, 0, 0}, message)

        if not StartLSD then
            Citizen.SetTimeout(cooldownDuration, function()
                ToggleCodeRunning()
                TriggerEvent("chatMessage", Config.SystemMessage, {255, 0, 0}, Config.EndLSD)
            end)
        end
    end
	sabrkn = false
	Wait(cooldownDuration + 1)
	sabrkn = true
else
	TriggerEvent("chatMessage", Config.SystemMessage, {255, 0, 0}, Config.BTime)
end
end
end)
